...
#     crawler = AsyncCrawler(link, max_concurrent_requests=15, domain_name="ada_conditions")
#     try:
...
class AsyncCrawler:
    def __init__(
        ...
        Args:
            ...
            client (str): The crawling client to use ("crawl4ai" or "scrapegraph"). Defaults to "crawl4ai".
            max_concurrent_requests (int): Maximum number of concurrent URL extraction requests.
            rate_delay (Optional[float]): Optional delay in seconds between requests to avoid rate limiting. Defaults to None (no delay).
        """
        output_folder = (
            Path(output_folder)
            if output_folder
            else create_folder.create(DATA_DIR) / "content"
        )
        self.output_file = output_folder / f"{domain_name}.txt"
        self.urls_file = output_folder / f"{domain_name}_urls.txt"
        self.client = client
        self.max_concurrent_requests = max_concurrent_requests
    ...
    async def extract_content(
        ...
        if webpage_only:
            ...
            if self.urls_file.exists():
                ...
                logger.info(f"Loaded {len(url_list)} URLs from {self.urls_file}")
            else:
                url_set = await self._extract_urls_fast(max_depth=max_depth)
                url_list = list(url_set)
                with open(self.urls_file, "w", encoding="utf-8") as f:
                    for url in url_list:
                        f.write(url + "\n")
                logger.info(f"Saved {len(url_list)} URLs to {self.urls_file}")
    ...
    async def _crawl_page_async(self, url: str, depth: int) -> List[tuple[str, int]]:
        ...
        try:
            async with self.session.get(url) as response:
                ...
                if self.rate_delay is not None:
